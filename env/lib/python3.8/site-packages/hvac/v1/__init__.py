...
class Client:
    ...
    def write(self, path, wrap_ttl=None, **kwargs):
        ...
        :rtype:
        """
        return post(f"/v1/{path}", json=kwargs, wrap_ttl=wrap_ttl)
    ...
    def lookup_token(self, token=None, accessor=False, wrap_ttl=None):
        ...
        if token:
            if accessor:
                ...
                return post(path, json=accessor_param, wrap_ttl=wrap_ttl)
            else:
                path = "/v1/auth/token/lookup"
                return post(path, json=token_param)
        else:
    ...
    def revoke_token(self, token, orphan=False, accessor=False):
        ...
        if accessor and orphan:
            ...
            post("/v1/auth/token/revoke-accessor", json=params)
        elif orphan:
            params = {"token": token}
            post("/v1/auth/token/revoke-orphan", json=params)
        else:
            params = {"token": token}
            post("/v1/auth/token/revoke", json=params)
